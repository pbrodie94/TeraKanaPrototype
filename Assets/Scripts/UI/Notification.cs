...
public class Notification : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        rect.anchoredPosition = Vector2.Lerp(rect.anchoredPosition, wantedPos, smoothSpeed * Time.fixedDeltaTime);

        //Alpha fade in
        Color color = notificationText.color;
        color.a = Mathf.Lerp(color.a, wantedColor.a, (smoothSpeed / 4) * Time.fixedDeltaTime);
        notificationText.color = color;

        if (_expired && Time.unscaledTime >= timeExpired + 0.1f)
        {
            _destroy = true;
        }
    }
